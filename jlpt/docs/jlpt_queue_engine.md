...
현재 앱의 프리셋은 `scripts/generate-app-translated-data.mjs`가 생성한 `imported.ts` 기준으로 적재된다. `N1`은 별도 정책을 사용하고, 다른 레벨 데이터가 추가되면 기존 non-N1 정책을 따른다.

### N1 생성 순서

N1은 `micro` 세트를 만들지 않는다. 현재 N1 단어 수는 `2699`개이며 UI 라벨 기준 세트는 아래 순서만 사용한다.
...
DB 범위는 기존 half-open 스타일을 유지한다. 예를 들어 UI 라벨 `1-300`은 `range_start = 0`, `range_end = 300`이고, 실제 단어 조회 조건은 `sequence_in_level > range_start AND sequence_in_level <= range_end`다.

### Non-N1 생성 순서

현재 데이터에는 N1만 포함되어 있지만, 스크립트에는 non-N1 fallback 정책이 남아 있다.

1. 먼저 `0-50`, `0-100`, `0-150`, `0-200`, `0-250`, `0-260`, `0-300` 누적 `micro` 구간을 가능한 만큼 만든다.
2. 그 이후에는 `300` 단위로 `block`과 `merge`를 번갈아 추가한다.
3. 마지막 꼬리 구간이 있으면 `block previousUpper-count`, `merge 0-count`를 추가한다.

### roundType 의미

- `micro`
  - non-N1 fallback 정책에서만 쓰는 초반 누적 진입 구간
- `block`
  - 독립 블록 범위. N1에서는 300개 블록 또는 마지막 꼬리 블록이다.
- `merge`
  - 여러 블록을 합친 범위. N1에서는 600개, 1200개, 누적 전체 범위를 포함한다.

앱 시작 시 N1의 기존 DB 프리셋 중 현재 `PRESET_SEEDS`에 없는 `preset_code`는 삭제한다. 따라서 예전 `micro` 세트가 로컬 SQLite에 남아 있어도 다음 초기화 이후 N1 화면에는 현재 17개 세트만 표시되어야 한다.
