...
const inputFile = typeof args.in === "string" ? args.in : `${PATHS.dataRoot}/tasks/translation.jobs.jsonl`;
...
for (const job of jobs) {
  const fileName = `${job.level.toLowerCase()}.translation.jsonl`;
  const record = {
    ...
    model,
    apiBase,
    meaningKoVariants: dryRun ? [`[stub:${job.level}] ${job.input.meaningEnRaw}`] : [],
...
console.log(`[openai-translation] jobs=${jobs.length} dryRun=${dryRun}`);
